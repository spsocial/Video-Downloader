...
app.post('/api/video-info', async (req, res) => {
    ...
    try {
        ...
        if (error.message.includes('Sign in to confirm') || error.message.includes('not a bot')) {
            if (url.includes('youtube.com') || url.includes('youtu.be')) {
                errorMessage = '⚠️ YouTube Shorts กำลังบล็อก bot detection ชั่วคราว\n\n' +
                    '💡 แนะนำ:\n' +
                    '• ลองใช้ YouTube วิดีโอปกติ (ไม่ใช่ Shorts) แทน\n' +
                    '• ใช้ TikTok, Facebook, Instagram แทน ยังใช้ได้ปกติ\n' +
                    '• หรือรอ YouTube ผ่อนคลายการตรวจจับ (อาจใช้เวลา 1-2 ชม.)\n\n' +
                    '📝 หมายเหตุ: YouTube อัปเดตระบบป้องกัน bot บ่อยมาก ทำให้บางช่วงเวลาอาจดาวน์โหลดไม่ได้';
            }
...
app.post('/api/download', async (req, res) => {
    ...
    try {
        ...
        } else if (error.message.includes('ERROR: Unsupported URL')) {
